    ...
    def decorator(view_func: Callable):
        def wrapper(request: HttpRequest, *args, **kwargs):
            ...
            messages.add_message(
                request,
                constants.WARNING,
                "É necessario estar logado para acessar esta página.",
            )
        ...
        def wrapper(request: HttpRequest, *args, **kwargs):
            if request.user.is_authenticated:
                ...
                messages.add_message(
                    request,
                    constants.WARNING,
                    "O usuário precisa do tipo loja para acessar esta página.",
                )
                return redirect(login_url)
        ...
        def wrapper(request: HttpRequest, *args, **kwargs):
            if request.user.is_authenticated:
                messages.add_message(
                    request,
                    constants.WARNING,
                    "É necessario estar deslogado para acessar esta página.",
                )
