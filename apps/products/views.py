    ...
    match request.method:
        case "GET":
            ...
            return render(
                request,
                "products.html",
                {"product_add_form": product_add_form, "products": products},
            )
...
def info_product(request: HttpRequest, product_id: int) -> HttpResponse:
    ...
    if not product.exists():
        messages.add_message(request, constants.WARNING, "Produto acessado não existe.")
        return redirect("products")
    if product.first().store.user != request.user:
        messages.add_message(
            request, constants.WARNING, "Produto acessado não pertence à esta loja."
        )
        return redirect("products")
...
def edit_product(request: HttpRequest, product_id: int) -> HttpResponse:
    ...
    match request.method:
        case "GET":
            ...
            return render(
                request, "edit_product.html", {"product_edit_form": product_edit_form}
            )
...
def del_product(request: HttpRequest, product_id: int) -> HttpResponse:
    ...
    match request.method:
        case "GET":
            ...
            product.delete()
            messages.add_message(
                request, constants.WARNING, f'Produto "{nome}" excluído.'
            )
            return redirect("products")
...
def exportar_etiqueta(request: HttpRequest, product_id: int) -> HttpResponse:
    match request.method:
        case "GET":
            ...
            pdf = PDF()
            pdf.add_product(qr_code_path, product.name, product.get_formated_price())
    ...
    match request.method:
        case "GET":
            ...
            if not checked_products:
                messages.add_message(
                    request, constants.ERROR, "Nenhum produto selecionado."
                )
            ...
            for product in products:
                ...
                generate_qr_code(product.id, qr_code_path)
                pdf.add_product(
                    qr_code_path, product.name, product.get_formated_price()
                )
                os.remove(qr_code_path)
